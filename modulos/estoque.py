...
def render(perfil_atual):
    ...
    try:
        ...
        df_user = pd.read_sql_query("SELECT perfil FROM usuarios WHERE username = %s OR usuario = %s", conn_p, params=(str(user_token), str(user_token)))
        conn_p.close()
        ...
        try:
            ...
            if not df_estoque.empty:
                ...
                if is_almoxarife_ou_gestao:
                    st.markdown("---")
                    with st.expander("✏️ Painel de Edição de Produtos Existentes", expanded=True):
                            ...
                            try:
                                ...
                                for _, row in edited_df.iterrows():
                                    ...
                                    """, (
                                        ...
                                        str(row['unidade']), float(row['preco_unitario']), str(row['localizacao']),
    ...
    with tab3:
        ...
        try:
            ...
            if df_estoque.empty:
                ...
                with col_b1:
                    item_selecionado = st.selectbox("Selecione o Item para Adicionar ao Carrinho", options=df_estoque.index, format_func=lambda x: f"{df_estoque.loc[x, 'codigo']} - {df_estoque.loc[x, 'item']} (Disp: {df_estoque.loc[x, 'quantidade']} | Loc: {df_estoque.loc[x, 'localizacao']})")
                with col_b2:
                ...
                if st.button("➕ Adicionar ao Carrinho"):
                    ...
                    if qtd_baixa > it['quantidade']:
                        ...
                        st.session_state["carrinho_baixa"].append({"id": int(it['id']), "codigo": it['codigo'], "item": it['item'], "quantidade": float(qtd_baixa), "unidade": it['unidade'], "localizacao": it['localizacao']})
                            ...
                            if not destino_baixa.strip():
                                ...
                                try:
                                    ...
                                    for item in st.session_state["carrinho_baixa"]:
                                        ...
                                        cursor.execute("INSERT INTO historico_estoque (tipo_movimentacao, item, quantidade, unidade, localizacao, projeto_motivo, retirado_por, usuario_sistema) VALUES (%s, %s, %s, %s, %s, %s, %s, %s);", ('BAIXA', item["item"], item["quantidade"], item.get('unidade', 'UN'), item["localizacao"], destino_baixa, destino_baixa, usuario_atual))
                                    conn.commit()
                                    cursor.close()
                                    conn.close()
            ...
            if btn_salvar_manual:
                if not cod_man or not desc_man or qtd_man <= 0:
                    ...
                    try:
                        ...
                        usuario_atual = st.session_state.get("user_token", "Sistema")
                        if existe:
                            cursor.execute("UPDATE estoque SET quantidade = quantidade + %s, preco_unitario = %s, ultimo_fornecedor = %s, localizacao = %s WHERE codigo = %s;", (float(qtd_man), float(preco_man), fornecedor_man, loc_inicial, cod_man))
                        else:
                            cursor.execute("INSERT INTO estoque (codigo, item, quantidade, unidade, preco_unitario, ultimo_fornecedor, localizacao) VALUES (%s, %s, %s, %s, %s, %s, %s);", (cod_man, desc_man, float(qtd_man), un_man, float(preco_man), fornecedor_man, loc_inicial))
                        cursor.execute("INSERT INTO historico_estoque (tipo_movimentacao, item, quantidade, unidade, localizacao, projeto_motivo, retirado_por, usuario_sistema) VALUES (%s, %s, %s, %s, %s, %s, %s, %s);", ('ENTRADA MANUAL', desc_man, float(qtd_man), un_man, loc_inicial, proj_man, 'Manual', usuario_atual))
                        conn.commit()
                        cursor.close()
                        conn.close()
    ...
    with tab5:
        ...
        arquivo_xml = st.file_uploader("Faça o upload do arquivo XML da NF-e", type=["xml"], key="uploader_xml_ativo" if not st.session_state["limpar_xml"] else "uploader_xml_limpo")
                    ...
                    if btn_confirma_xml:
                        try:
                            ...
                            for item in lista_produtos:
                                ...
                                existe = cursor.fetchone()
                                if existe:
                                    cursor.execute("UPDATE estoque SET quantidade = quantidade + %s, preco_unitario = %s, ultimo_fornecedor = %s, localizacao = %s WHERE codigo = %s;", (item_qtd, item_preco, item["fornecedor"], loc_recebido, item["codigo"]))
                                else:
                                    cursor.execute("INSERT INTO estoque (codigo, item, quantidade, unidade, preco_unitario, ultimo_fornecedor, localizacao) VALUES (%s, %s, %s, %s, %s, %s, %s);", (item["codigo"], item["descricao"], item_qtd, item["unidade"], item_preco, item["fornecedor"], loc_recebido))
                                cursor.execute("INSERT INTO historico_estoque (tipo_movimentacao, item, quantidade, unidade, localizacao, projeto_motivo, retirado_por, usuario_sistema) VALUES (%s, %s, %s, %s, %s, %s, %s, %s);", ('ENTRADA XML', item["descricao"], item_qtd, item["unidade"], loc_recebido, projeto_destino, 'Fiscal', usuario_atual))
                            conn.commit()
                            cursor.close()
                            conn.close()
